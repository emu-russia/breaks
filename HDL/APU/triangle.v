...
module TriangleChan(
	...
	TRIANGLE_Control ctrl (.PHI1(PHI1), .ACLK1(ACLK1), .W4008(W4008), .W400B(W400B), .n_LFO1(nLFO1), .NOTRI(NOTRI), .LOCK(LOCK), .TCO(TCO), .FOUT(FOUT),
		.DB(DB), .TRI_LC(TRI_LC), .TLOAD(TLOAD), .TSTEP(TSTEP), .TFLOAD(TFLOAD), .TFSTEP(TFSTEP), .TTSTEP(TTSTEP) );

	TRIANGLE_LinearCounter lin_cnt (.ACLK1(ACLK1), .RES(RES), .W4008(W4008), .TLOAD(TLOAD), .TSTEP(TSTEP), .DB(DB), .TCO(TCO) );

	TRIANGLE_FreqCounter freq_cnt (.PHI1(PHI1), .RES(RES), .W400A(W400A), .W400B(W400B), .DB(DB), .TFLOAD(TFLOAD), .TFSTEP(TFSTEP), .FOUT(FOUT) );

	TRIANGLE_Output tri_out (.PHI1(PHI1), .RES(RES), .W401A(W401A), .TTSTEP(TTSTEP), .DB(DB), .TRI_Out(TRI_Out) );

endmodule // TriangleChan

module TRIANGLE_Control (PHI1, ACLK1, W4008, W400B, n_LFO1, NOTRI, LOCK, TCO, FOUT, DB, TRI_LC, TLOAD, TSTEP, TFLOAD, TFSTEP, TTSTEP);
	...
	nor (TFLOAD, PHI1, n_FOUT);
	nor (TFSTEP, PHI1, ~n_FOUT);
	...
	nor (TLOAD, n_LFO1, reload_latch2_nq);
	nor (TSTEP, n_LFO1, reload_latch2_q, tco_latch_q);
	nor (TTSTEP, TCO, LOCK, PHI1, NOTRI, n_FOUT);

endmodule // TRIANGLE_Control

module TRIANGLE_LinearCounter (ACLK1, RES, W4008, TLOAD, TSTEP, DB, TCO);
	...
	DownCounterBit lin_cnt [6:0] (.ACLK1(ACLK1), .d(lq), .load(TLOAD), .clear(RES), .step(TSTEP), .cin({cout[5:0],1'b1}), .cout(cout) );
	assign TCO = cout[6];

endmodule // TRIANGLE_LinearCounter

module TRIANGLE_FreqCounter (PHI1, RES, W400A, W400B, DB, TFLOAD, TFSTEP, FOUT);
	...
	DownCounterBit freq_cnt [10:0] (.ACLK1(PHI1), .d(fq), .load(TFLOAD), .clear(RES), .step(TFSTEP), .cin({cout[9:0],1'b1}), .cout(cout) );
	assign FOUT = cout[10];

endmodule // TRIANGLE_FreqCounter

module TRIANGLE_Output (PHI1, RES, W401A, TTSTEP, DB, TRI_Out);
