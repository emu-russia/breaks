...
static PLA_ENTRY PLA_ROM[130] = {     // 129 external lines + 1 internal
    ...
    { "010000010110000100000", "BR_0" },  // XXX10000 T0 <-  Branch + #PRDY line
    ...
    { "000001000000000000000", "#IR6" },  // X0XXXXXX TX     IR 6 = 0
    ...
    { "000000000000010000000", "#IR7" },  // 0XXXXXXX TX     IR 7 = 0
    { "001001011010100100000", "CLV" },  // 10111000 TX     CLV
    { "000000011000000......", "IMPL" },  // XXXX10X0 TX     All implied, except Push/pull
    { "000000011001010100000", "Push/Pull" },   // 0XX01000 TX   Push/pull excluder

};

void PLAGen (unsigned char op, int nT[6])
{
    ...
    for (n=0; n<128; n++) {
        ...
        if ( nT[1] && line[0] == '1' ) out = 0;
        else if ( nT[0] && line[1] == '1' ) out = 0;
        ...
        else if ( nT[2] && line[17] == '1' ) out = 0;
        else if ( nT[3] && line[18] == '1' ) out = 0;
        else if ( nT[4] && line[19] == '1' ) out = 0;
        else if ( nT[5] && line[20] == '1' ) out = 0;

        // Line 73 has additional cutoff by #PRDY
//        else if ( n == 73 && cpu->ctrl[M6502_CTRL_nPRDY] ) out = 0;
    ...
    PLA[128] = NOT ( IR(2) | nIR(3) | IR(0) | PushPull );

    PLA[129] = PushPull;

    printf ( "    /* %02X:%s */ ", op, inames[op] );
    printf ("{");
    for (n=0;n<130;n++) printf ("%i,", PLA[n]);
    printf ("}, //");
    for (n=0;n<130;n++) {
        if (PLA[n]) printf ( "%s, ", PLA_ROM[n].name );
    }
    printf ("\n");
}
...
main ()
{
    ...
    printf ("char PLA_0[256][130] = {\n");
    for (op=0;op<=0xff;op++){
        nT[0] = 0; nT[1] = 1; nT[2] = 1; nT[3] = 1; nT[4] = 1; nT[5] = 1;
        PLAGen (op&0xff, nT);
    }
    printf ("};\n\n" );

    printf ("char PLA_1[256][130] = {\n");
    for (op=0;op<=0xff;op++){
        nT[0] = 1; nT[1] = 0; nT[2] = 1; nT[3] = 1; nT[4] = 1; nT[5] = 1;
        PLAGen (op&0xff, nT);
    }
    printf ("};\n\n" );

    printf ("char PLA_2[256][130] = {\n");
    for (op=0;op<=0xff;op++){
        nT[0] = 1; nT[1] = 1; nT[2] = 0; nT[3] = 1; nT[4] = 1; nT[5] = 1;
        PLAGen (op&0xff, nT);
    }
    printf ("};\n\n" );

    printf ("char PLA_3[256][130] = {\n");
    for (op=0;op<=0xff;op++){
        nT[0] = 1; nT[1] = 1; nT[2] = 1; nT[3] = 0; nT[4] = 1; nT[5] = 1;
        PLAGen (op&0xff, nT);
    }
    printf ("};\n\n" );

    printf ("char PLA_4[256][130] = {\n");
    for (op=0;op<=0xff;op++){
        nT[0] = 1; nT[1] = 1; nT[2] = 1; nT[3] = 1; nT[4] = 0; nT[5] = 1;
        PLAGen (op&0xff, nT);
    }
    printf ("};\n\n" );

    printf ("char PLA_5[256][130] = {\n");
    for (op=0;op<=0xff;op++){
        nT[0] = 1; nT[1] = 1; nT[2] = 1; nT[3] = 1; nT[4] = 1; nT[5] = 0;
        PLAGen (op&0xff, nT);
    }
    printf ("};\n\n" );

    printf ("char PLA_X[256][130] = {\n");
    for (op=0;op<=0xff;op++){
        nT[0] = 1; nT[1] = 1; nT[2] = 1; nT[3] = 1; nT[4] = 1; nT[5] = 1;
        PLAGen (op&0xff, nT);
    }
    printf ("};\n\n" );
}