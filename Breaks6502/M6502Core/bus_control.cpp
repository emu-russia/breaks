...
namespace M6502Core
{
	void BusControl::sim(BusControl *inst)
	{
		...
		if (PHI2 == TriState::One)
		{
			...
			TriState SBA = NOR(n_SB_ADH, NAND(ACRL2, inst->nready_latch.nget()));

			// External address bus control

			TriState n_ADL_ABL = NAND(NOR(T5, T6), NOR(NOT(NOR(d[71], d[72])), n_ready));
			inst->adl_abl_latch.set(n_ADL_ABL, PHI2);
			...
			inst->adh_abh_latch.set(n_ADH_ABH, PHI2);
			...
			sbac[6] = d[64];
			n_SB_AC = NOR7(sbac);
			inst->sb_ac_latch.set(n_SB_AC, PHI2);
			...
			inst->ac_sb_latch.set(n_AC_SB, PHI2);

			inst->ac_db_latch.set(NOR(d[74], AND(d[79], STOR)), PHI2);

			// Control of the SB, DB and ADH internal buses (SB/DB, SB/ADH, 0/ADH0, 0/ADH17)

			inst->z_adh0_latch.set(n_DL_ADL, PHI2);
			inst->z_adh17_latch.set(NOR(d[57], NOT(n_DL_ADL)), PHI2);
			...
			inst->sb_db_latch.set(n_SB_DB, PHI2);

			inst->sb_adh_latch.set(n_SB_ADH, PHI2);

			// External data bus control

			inst->dl_adh_latch.set(NOR(DL_PCH, IND), PHI2);
			inst->dl_adl_latch.set(n_DL_ADL, PHI2);
			...
			inst->dl_db_latch.set(n_DL_DB, PHI2);
		}
		else
		{
			TriState sbac[7];
		...
		core->cmd.ADL_ABL = inst->adl_abl_latch.nget();
		core->cmd.ADH_ABH = inst->adh_abh_latch.nget();

		core->cmd.AC_DB = NOR(inst->ac_db_latch.get(), PHI2);
		core->cmd.SB_AC = NOR(inst->sb_ac_latch.get(), PHI2);
		core->cmd.AC_SB = NOR(inst->ac_sb_latch.get(), PHI2);

		core->cmd.SB_DB = inst->sb_db_latch.nget();
		core->cmd.SB_ADH = inst->sb_adh_latch.nget();
		core->cmd.Z_ADH0 = inst->z_adh0_latch.nget();
		core->cmd.Z_ADH17 = inst->z_adh17_latch.nget();
