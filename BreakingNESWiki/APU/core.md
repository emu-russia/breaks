...
Не очень к месту, но сказать придется именно в этом месте, т.к. кроме хранения данных для спрайтовой DMA данная схема также занимается получением сигнала `WR` для конактов внешней шины данных, а также сигнала `RW`.
