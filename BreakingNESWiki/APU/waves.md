...
- Перед началом работы в счётчик загружается значение 0b1001 (9), которое после обработки на декодере соответствует значению 0x07.
...
С помощью этого теста мы пытаемся заставить блок Sweep генерировать сигнал DO_SWEEP так, как это должно быть для работы процесса Sweep.
...
и проверить, что сигнал DO_SWEEP генерируется так, как должен.
...
Начало OAM DMA (выровненное на ACLK1):

![oam_dma_start1](/BreakingNESWiki/imgstore/apu/waves/oam_dma_start1.png)

Начало OAM DMA (невыровненное на ACLK1, требуется 1 дополнительный цикл CPU):
