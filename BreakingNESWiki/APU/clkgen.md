...
|9|Выход сигналов ACLK1 и /ACLK2 с генератора ACLK (PHI ÷ 2)|
...
Сигналы `ACLK1` и `/ACLK2` не комплементарны и не перекрываются:
...
Кое-где в разных частях APU находятся так называемые "другие" ACLK2. По сути это инверсия сигнала /ACLK2, используемая "по месту". Но чтобы их разделять, они нумеруются в порядке возрастания: 2, 3 итд.

Вначале был обнаружен "другой" ACLK2 в схеме DPCM/DMA, но потом оказалось что они встречаются и в других частях APU. Поэтому эти сигналы называются по очередности 2, 3, 4 итд.

На схемах знаком :warning: отмечены места, где используется другие `ACLK2`.

### ACLK2

В самом центре схемы DPCM находится схема, для получения "другого" ACLK2, который используется в [DPCM](dpcm.md), а также в [спрайтовой DMA](dma.md).
