...
|---|---|---|---|
|ADDOUT|Sweep|Freq Reg|Основной сигнал, который управляет процессом Sweep значения частоты, загруженной в Freq Reg. Сигнал устанавливается только во время PHI1, чтобы не конфликтовать с записями в регистр со стороны CPU, которые происходят только во время PHI2.|
