...
Синхронизация между PPU осуществляется просто очередностью вывода и получения данных контактов EXT (см. [мультиплексор](mux.md)). При этом, согласно патенту, для дополнительной синхронизации PPU предлагается объединять их контакты /RES, чтобы все регистры и счетчики H/V сбрасывались одновременно.
