...
- Схема содержит 1 FF и 2 защёлки
- Входное значение с ячейки памяти в виде комплементарного значения BL+BLBar поступает на вход Sense Amp (дифференциальный компаратор). На его входе происходит "борьба" уровней сигнала и "побеждает" тот NOR, где уровень сигнала выше (1). Во время PCLK выход Sense Amp закорочен pass-gate транзистором (двунаправленный FET), предположительно чтобы быстрее подготовить его к очередному циклу чтения.
...
- `Защёлка R4` (DLatch) используется для CPU I/F. В ней хранится значение для чтения регистра $2004. Открывается защёлка во время PCLK = 1.
- Выходная защёлка (`out_latch`) (DLatch) используется для записи нового значения в OAM Cell. Новое значение может прийти с OB_FF (`OB/OAM` = 1) или с шины данных (`BLNK` = 1). Сигналы `OB/OAM` и `BLNK` никогда не принимают одновременно значение `1`, но могут принимать одновременно значение `0` (то есть когда выходная защёлка закрыта).
