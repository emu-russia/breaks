...
Что касается видеовыхода, то на базе схемы Phase Shifter и значений Chroma декодера теперь есть возможность сделать фильтр/шейдер наподобии NTSC Filter by blargg. Опытный NES-хакер тут легко справиться.

Ещё одна тайна - мы не смогли найти в каком месте перепутаны разряды Emphasis. Значения с блока регистров выходят НЕ в перепутанном виде, а значит они перепутаны где-то в Phase Shifter.
