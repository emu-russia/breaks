...
Специальным значком (:zap:) отмечены важные управляющие сигналы PPU [FSM](hv_fsm.md).
...
|V0-7|VCounter|Sprite Compare, Sprite Eval|Разряды счетчика V. Разряд V7 дополнительно уходит в логику Sprite Eval.|
...
|SPR_OV|OAM Counters Ctrl|Sprite Eval|Переполнение счетчика OAM|
...
|:zap:CLIP_O|FSM|Control Regs|"Clip Objects". Не показывать левые 8 точек экрана для спрайтов. Используется для получения сигнала `CLPO`, который уходит в OAM FIFO.|
|:zap:CLIP_B|FSM|Control Regs|"Clip Background". Не показывать левые 8 точек экрана для бэкграунда. Используется для получения сигнала `CLPB`, который уходит в Data Reader.|
...
|:zap:0/HPOS|FSM|OAM FIFO|"Clear HPos". Очистить счетчики H в спрайтовой FIFO и начать работу FIFO|
|I2SEV|Sprite Eval|Spr0 Stike|Для определения события `Sprite 0 Hit`|
