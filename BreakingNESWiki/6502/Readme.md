...
В обоих случаях верхнюю часть занимает декодер и рандомная логика, а всю нижнюю часть процессора занимает контекст.
...
## Программная модель 6502

### Режимы адресации 6502

Описание режимов адресации тут приводится по причине того, что его нужно держать в голове при анализе схем.
...
Прерывания 6502:
- IRQ: обычное аппаратное прерывание. Можно запретить флагом I (interrupt disable), если флаг I=1, то прерывание "отключено" и не проходит в процессор. 
...
Транзисторные схемы каждого компонента безжалостно напилены на составные части, чтобы не занимали много места на экране.
