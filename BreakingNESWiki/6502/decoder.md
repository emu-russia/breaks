...
Схематически каждый выход представляет собой многовходовый элемент NOR.
...
|F|||||||
|F01  |73 |010000010110000100000   |XXX10000 |T0 |<-  Branch. :warning: Дополнительно влияет линия /PRDY (с контакта RDY), сразу на месте|Вспомогательный сигнал BR0|
...
|P/P |129 |000000011001010100000   |0XX01000 |TX |<-  Опкоды Push/pull, применяется как эксклюдер для F11 & F18 (используется изнутри)|Первый экземпляр pp|
...
|K|||||||
|P/P |129 |000000011001010100000   |0XX01000 |TX     |<-  Опкоды Push/pull, применяется как эксклюдер для K09 (используется изнутри)|Второй экземпляр pp|
...
|K09 |128 |000000011000000000000   |XXXX10X0 |TX     |IMPL. :warning: К этой линии дополнительно применяется операция исключения Push/Pull опкодов, сразу на месте. Также маска для этой линии не учитывает операцию `& ~IR0`|Bus Control (DL/DB)|
...
Для примера, на картинке представлена 5-я линейка декодера. Порядок бит на картинке следует снизу-вверх от старших разрядов к младшим (msb -> lsb). 0 означает отсутствие транзистора, 1 - наличие.
